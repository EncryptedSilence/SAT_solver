"""DDT-aware minimum-weight differential trail for lin344, c0=(1,10,15), R=4.

Self-contained script with INCREMENTAL solving:

  * The CNF encoding — byte activity, S-box bijection, XOR-exact linear
    layer, iv/ov indicators, AES DDT pair clauses — is built ONCE.
...
# Allow running directly via `python examples/best_trail_c0_1_10_15_R4.py`.
...
# Standard AES S-box (FIPS 197). Differential uniformity = 4, so DDT
...
def main() -> None:
    ...
    # per-S-box weight is 6 (DDT=4) or 7 (DDT=2) in the AES DDT
    # (values in {0, 2, 4, 256}).
    ...
    # Known UNSAT from prior run: W=78,81,83. Skip straight to 84.
    w_lo = 7 * A
    w_hi = 7 * A
